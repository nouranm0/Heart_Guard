...
from app.validation_utils import normalize_email, normalize_phone, validate_email, validate_password_strength, validate_phone
...
def get_dashboard_stats():
    ...
    if user.role == 'admin':
        ...
        for patient in patients:
            latest_record = latest_records_by_patient.get(patient.id)
            assessment_count = len(patient.ecg_records)
            latest_confidence = float(latest_record.top_confidence) if latest_record and latest_record.top_confidence is not None else None
            risk = _classify_patient_risk(assessment_count, latest_record.top_diagnosis if latest_record else None, latest_confidence)
...
def update_user_settings():
    ...
    try:
        ...
        if email:
            ...
            existing_user = User.query.filter(db.func.lower(User.email) == email, User.id != user.id).first()
            if existing_user:
        ...
        if 'phone' in data:
            if phone and not validate_phone(phone):
                return {'success': False, 'message': 'Please enter a valid phone number'}, 400
